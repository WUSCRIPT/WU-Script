...
Tabs.Home:Button({
    ...
    Callback = function()
        ...
        createFloatingText(LP, "EALPHA X 用户")
...
local Window = WindUI:CreateWindow({
    Title = "Ealpha X<font color='#00FF00'>Translation</font>",
...
WindUI:Notify({
    ...
    Duration = 3
       ...
       Library:Notify("EALPHA X 自动翻译 已加载!", 5)
    end
})
-- WU偷走脑红功能
...
local Window = Library:CreateWindow({
    Title = "EALPHA X DOORS",
    Footer = "誓约 |  EALPHA X  | Floor: " .. currentFloor,
...
ChangelogsGroup:AddLabel('<font color="rgb(0,255,0)"> EALPHA X</font>')
...
Library:Notify("EALPHA X DOORS 已加载!", 5)
    end
})
...
SaveManager:LoadAutoloadConfig()
        ...
        WindUI:Notify({Title = "Ealpha X被遗弃", Content = "加载成功", Duration = 3})
    end
})
...
task.wait(1)
WindUI:Notify({
    Title = "Ealpha X 已加载完毕",
    Content = "Ealpha X UI汉化",
    Icon = "check-circle",
    Duration = 3
})
        Library:Notify("Ealpha X自动翻译已加载", 3)
    end
})
...
task.spawn(function()
    while not Unloaded do
        ...
        Library:Notify("Ealpha X DOORS 已加载", 3)
    end
})

ScriptLeftGroup:AddButton({
    Text = "Ealpha X Forsken",
    Func = function()
...
Library:Notify("EALPHA X 加载完成!", 5)
        Library:Destroy()
    end
})